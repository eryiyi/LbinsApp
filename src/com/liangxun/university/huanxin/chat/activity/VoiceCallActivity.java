...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cover = (NetworkImageView) findViewById(R.id.swing_card);
        ...
        if (user != null) {
            username = user.getHxUsername();
            userNick = user.getEmpName();
            cover.setDefaultImageResId(R.drawable.default_image);
            cover.setErrorImageResId(R.drawable.default_image);
            cover.setImageUrl(user == null ? "" : user.getEmpCover(), imageLoader);
        } else {
            username = getIntent().getStringExtra("username");
            getNickNamesByHxUserNames(username);
        }
                    ...
                    public void onResponse(String s) {
                        if (StringUtil.isJson(s)) {
                            ...
                            if (data.getCode() == 200) {
                                ...
                                nickTextView.setText(user.getEmpName());
                                cover.setDefaultImageResId(R.drawable.default_image);
                                cover.setErrorImageResId(R.drawable.default_image);
                                cover.setImageUrl(user == null ? "" : user.getEmpCover(), imageLoader);
                            } else {
                                Toast.makeText(VoiceCallActivity.this, "获得数据失败，请稍后重试", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(VoiceCallActivity.this, "获得数据失败，请稍后重试", Toast.LENGTH_SHORT).show();
                        }
